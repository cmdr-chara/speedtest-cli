    ...
    try:
        with tempfile.TemporaryDirectory(prefix='cockpit-smoke-') as scratch:
            ...
            with session(options) as tty:
                ...
                tty.wait('Compact')
                tty.snapshot('Appearance controls filling a 180x48 terminal')
                tty.resize(80, 24)
                tty.wait('READABILITY')
                tty.send('\r')
            ...
            with session(options) as tty:
                ...
                tty.send('\x1b')
                tty.wait('YOUR NETWORK, OVER TIME')
                tty.send('\x1b[4~')  # End selects the oldest saved run.
                tty.wait('fixture-00')
                tty.send('\r')
                tty.wait('MEASUREMENT COMPLETE')
                tty.wait('fixture-00')
                ...
                tty.send('b')
                tty.wait('BASELINE')
                tty.send('\x1b[1~c')  # Home chooses newest AFTER, pinned oldest remains BEFORE.
                tty.wait('BEFORE / AFTER')
                tty.wait('fixture-00')
                tty.wait('fixture-24')
                tty.snapshot('Pinned oldest baseline compared with selected newest run')
                tty.send('\x1b')
                tty.wait('YOUR NETWORK, OVER TIME')
                newest = dict(result)
                newest['timestamp'] = datetime.datetime.now(datetime.timezone.utc).isoformat()
                newest['backend'] = 'fixture-25'
                ...
                tty.wait('MEASUREMENT COMPLETE')
                tty.wait('fixture-24')
                assert 'fixture-25' not in tty.screen.text(), 'reload changed the selected run identity'
                tty.send('\x1b')
                tty.wait('YOUR NETWORK, OVER TIME')
                tty.wait('BASELINE')
                tty.send('c')
